...
func (ur *UserRepository) GetAll(ctx context.Context) ([]user.User, error) {
	...
	defer rows.Close() // with `defer`, the cursor will be close
	var users []user.User
	for rows.Next() {
		var u user.User
...
func (ur *UserRepository) Create(ctx context.Context, u *user.User) error {
	...
	row := ur.Data.DB.QueryRowContext(ctx, q,
		u.FirstName, u.LastName, u.Username, u.Email, u.Picture, u.PasswordHash, time.Now(), time.Now())
	err := row.Scan(&u.ID)
	if err != nil {
		return err
	}
...
func (ur *UserRepository) Update(ctx context.Context, id uint, u *user.User) error {
	...
	defer stmt.Close()
	_, err = stmt.ExecContext(ctx, u.FirstName, u.LastName, u.Email, u.Picture, time.Now(), id)
	if err != nil {
		return err
	}
	return nil
